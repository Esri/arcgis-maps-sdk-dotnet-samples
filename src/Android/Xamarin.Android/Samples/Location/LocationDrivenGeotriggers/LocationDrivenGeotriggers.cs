    ...
    public class LocationDrivenGeotriggers : Activity
    {
        ...
        private void HandleGeotriggerNotification(object sender, GeotriggerNotificationInfo info)
        {
            ...
            RunOnUiThread(async () =>
            {
                    ...
                    if (fenceInfo.FenceNotificationType == FenceNotificationType.Entered)
                    {
                        if (!_features.Any(f => f.Name == fenceInfo.Message))
                        {
                            try
                            {
                                ...
                                // Get the image data.
                                System.IO.Stream contentStream = await attach.First().GetDataAsync();
                                ...
                                Bitmap imageBitmap = BitmapFactory.DecodeByteArray(attachmentData, 0, attachmentData.Length);
                ...
                _sectionButton.Text = $"Section: {_currentSection}";

                if (_pointsOfInterest.Any())
                {
                    _poiLabel.Text = $"Points of interest: {string.Join(", ", _pointsOfInterest)}";
                }
                else
                {
                    _poiLabel.Text = $"Points of interest:";
                }
            });
        }
        ...
        private void DisplayData(GeotriggerFeature feature)
        {
            ...
            image.SetImageBitmap(feature.ImageBitmap);
