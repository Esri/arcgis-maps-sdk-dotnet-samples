...
namespace ArcGIS.Samples.AnalyzeTerrainSuitabilityFromSlopeAndAspect
{
    ...
    public partial class AnalyzeTerrainSuitabilityFromSlopeAndAspect : ContentPage
    {
        ...
        private async Task Initialize()
        {
            ...
            try
            {
                ...
                _lowlandSouthFacingSlopesAnalysis = CreateScenarioAnalysis(
                    ...
                    elevationMin: 0,
                    elevationMax: 300,
                    color: System.Drawing.Color.FromArgb(76, 175, 80)); // Green.
        ...
        private FieldAnalysis CreateScenarioAnalysis(float slopeMin, float slopeMax, float aspectStart, float aspectEnd, float elevationMin, float elevationMax, System.Drawing.Color color)
        {
            ...
            Colormap colormap = Colormap.Create(new[] { System.Drawing.Color.White, color });
        ...
        private FieldAnalysis SelectedScenarioAnalysis => LowlandSouthFacingSlopesRadioButton.IsChecked
            ? _lowlandSouthFacingSlopesAnalysis
            : _uplandWestToNorthFacingSlopesAnalysis;

        private void OnScenarioSelectionChanged(object sender, CheckedChangedEventArgs e)
        {
            // Both radio buttons raise the event when the selection changes, so only act on the one that was checked.
            if (!e.Value) return;
        ...
        {
            // The radio buttons raise their CheckedChanged event while the sample is loading, before the analyses are created.
            if (_lowlandSouthFacingSlopesAnalysis == null) return;

            // Show only the analysis for the selected scenario.
            bool showLowlandSouthFacingSlopes = LowlandSouthFacingSlopesRadioButton.IsChecked;
        ...
        private async void OnAnalysisViewStateChanged(object sender, AnalysisViewStateChangedEventArgs e)
        {
            ...
            // Show an activity indicator while the analysis is being computed.
            AnalysisActivityIndicator.IsVisible = e.AnalysisViewState.Status == AnalysisViewStatus.Updating;
            ...
            if (e.AnalysisViewState.Status == AnalysisViewStatus.Error && !_analysisErrorReported)
            {
                ...
                await Application.Current.Windows[0].Page.DisplayAlertAsync("Error displaying analysis", e.AnalysisViewState.Error?.Message, "OK");
            }
        }
    }
}
