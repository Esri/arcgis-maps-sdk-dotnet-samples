...
namespace ArcGIS.Samples.StatsQueryGroupAndSort
{
    ...
    public partial class StatsQueryGroupAndSort : ContentPage
    {
        ...
        private ObservableCollection<ResultGroup> _resultsGroupCollection = new ObservableCollection<ResultGroup>();

        // Collection to hold the results.
        public ObservableCollection<ResultGroup> ResultsGroupCollection { get; private set; } = new ObservableCollection<ResultGroup>();
        ...
        private void StatResultsList_RemainingItemsThresholdReached(object sender, EventArgs e)
        {
            ...
            int startIndex = ResultsGroupCollection.Count;

            // End index will be the start index plus 15 or the total stats, whichever is smaller.
            int endIndex = Math.Min(startIndex + 100, _resultsGroupCollection.Count);

            // Add the items to the results collection.
