    ...
    public partial class FeatureCollectionLayerFromPortal : ContentPage
    {
        ...
        private async Task OpenFeaturesFromArcGISOnline(string itemId)
        {
            try
            {
                ...
                {
                    await Application.Current.Windows[0].Page.DisplayAlertAsync("Feature Collection", "Portal item with ID '" + itemId + "' is not a feature collection.", "OK");
                }
            }
            catch (Exception ex)
            {
                await Application.Current.Windows[0].Page.DisplayAlertAsync("Error", "Unable to open item with ID '" + itemId + "': " + ex.Message, "OK");
            }
        }
        ...
        private void OpenPortalFeatureCollectionClick(object sender, EventArgs e)
        {
            ...
            {
                _ = DisplayAlertAsync("Feature Collection ID", "Please enter a portal item ID", "OK");
                return;
            }
