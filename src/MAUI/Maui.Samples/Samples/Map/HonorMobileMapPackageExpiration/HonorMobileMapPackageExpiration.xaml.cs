        ...
        private async Task Initialize()
        {
            try
            {
                ...
                if (mobileMapPackage.Expiration?.IsExpired == true)
                {
                    ...
                    {
                        await Application.Current.Windows[0].Page.DisplayAlertAsync("Error", "The author of this mobile map package has disallowed access after the expiration date.", "OK");
                    }
                }
                ...
                {
                    await Application.Current.Windows[0].Page.DisplayAlertAsync("Error", "Failed to load the mobile map package.", "OK");
                }
            }
            catch (Exception e)
            {
