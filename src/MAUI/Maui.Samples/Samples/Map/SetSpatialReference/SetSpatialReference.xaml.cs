...
namespace ArcGIS.Samples.SetSpatialReference
{
    ...
    public partial class SetSpatialReference : ContentPage
    {
        ...
        private void Initialize()
        {
            ...
            // Populate the picker with spatial reference names.
            SpatialReferencePicker.ItemsSource = _spatialReferenceOptions.Select(sr => sr.Name).ToList();

            // Set the default selection to World Bonne.
            SpatialReferencePicker.SelectedIndex = _spatialReferenceOptions.FindIndex(sr => sr.Wkid == 54024);
        }

        private void SpatialReferencePicker_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (SpatialReferencePicker.SelectedIndex >= 0)
            {
                var selectedOption = _spatialReferenceOptions[SpatialReferencePicker.SelectedIndex];
    ...
    /// <summary>
    /// Represents a spatial reference option for display in the picker.
    /// </summary>
