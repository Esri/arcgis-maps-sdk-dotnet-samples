    ...
    public partial class OfflineGeocode : ContentPage
    {
        ...
        private async Task UpdateSearchAsync()
        {
            ...
            try
            {
                ...
                {
                    await Application.Current.Windows[0].Page.DisplayAlertAsync("No results", "No results found.", "OK");
                    return;
                }
        ...
        private async void SuggestionButtonTapped(object sender, System.EventArgs e)
        {
            try
            {
                ...
                string action = await Application.Current.Windows[0].Page.DisplayActionSheetAsync("Choose an address to geocode", "Cancel", null, _addresses);
                // Update the search
