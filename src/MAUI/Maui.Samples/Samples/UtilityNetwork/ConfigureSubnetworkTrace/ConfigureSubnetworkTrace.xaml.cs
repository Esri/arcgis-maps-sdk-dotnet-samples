    ...
    public partial class ConfigureSubnetworkTrace : ContentPage
    {
        ...
        private async void OnTrace(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                await Application.Current.Windows[0].Page.DisplayAlertAsync("Trace Result", $"`{elementResult?.Elements?.Count ?? 0}` elements found.", "OK");
            }
            catch (Exception ex)
            {
                await Application.Current.Windows[0].Page.DisplayAlertAsync("Error", $"{ex.Message}\nFor a working barrier condition, try \"Transformer Load\" Equal \"15\".", "OK");
            }
        }
