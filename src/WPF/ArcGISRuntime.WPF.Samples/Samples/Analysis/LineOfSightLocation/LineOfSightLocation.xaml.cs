...
namespace ArcGISRuntime.WPF.Samples.LineOfSightLocation
{
    ...
    public partial class LineOfSightLocation
    {
        ...
        public LineOfSightLocation()
        {
            ...
            // Handle pointer move to show a temporary line of sight target
            MySceneView.MouseMove += SceneViewMouseMove;
        }
        ...
        private async void SceneViewMouseMove(object sender, System.Windows.Input.MouseEventArgs e)
        {
            // If there's an observer, but a target location hasn't been defined, use the pointer location as the line of sight target
            if (_observerLocation != null && _targetLocation == null)
            {
                System.Windows.Point screenPoint = e.GetPosition(MySceneView);
                MapPoint mouseMapPoint = await MySceneView.ScreenToLocationAsync(screenPoint);
                _lineOfSightAnalysis.TargetLocation = new MapPoint(mouseMapPoint.X, mouseMapPoint.Y, mouseMapPoint.Z + _zOffset);
            }
        }
