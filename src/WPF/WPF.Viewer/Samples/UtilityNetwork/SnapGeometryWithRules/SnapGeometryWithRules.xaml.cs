...
namespace ArcGIS.WPF.Samples.SnapGeometryWithRules
{
    ...
    public partial class SnapGeometryWithRules : INotifyPropertyChanged
    {
        ...
        private async Task SetSnapSettings()
        {
            if (_selectedFeature is ArcGISFeature feature && _utilityNetwork != null)
            {
                await _utilityNetwork.LoadAsync();
                var element = _utilityNetwork.CreateElement(feature);
                var snapRules = await SnapRules.CreateSnapRulesAsync(_utilityNetwork, element.AssetType);
                _geometryEditor.SnapSettings.SyncSourceSettings(snapRules);

                foreach (var o in MyMapView.Map.OperationalLayers)
                {
                    if (o is ISnapSource source && o is ILayerContent layerContent)
                    {
                        var rulePolicy = snapRules.GetRulePolicy(source);
                        Debug.WriteLine($"RulePolicy on LayerContent.Name ({layerContent.Name}) : {rulePolicy}");
                    }
                }
            }
        ...
        private void UpdateSymbology()
        {
            ...
            var element = _utilityNetwork.CreateElement((ArcGISFeature)_selectedFeature);

            switch (element.AssetType.Name)
            {
                case "Automatic Reset":
                    _distributionPipeSubtypeSublayer.Renderer = new SimpleRenderer(new SimpleLineSymbol(SimpleLineSymbolStyle.Solid, System.Drawing.Color.Red, 4));
                    _servicePipeSubtypeSublayer.Renderer = new SimpleRenderer(new SimpleLineSymbol(SimpleLineSymbolStyle.Solid, System.Drawing.Color.Orange, 3));
                ...
                case "Metal 3-Way":
                    ...
                    break;
                case "Plastic End Cap":
                    _distributionPipeSubtypeSublayer.Renderer = new SimpleRenderer(new SimpleLineSymbol(SimpleLineSymbolStyle.Solid, System.Drawing.Color.Orange, 4));
                    _servicePipeSubtypeSublayer.Renderer = new SimpleRenderer(new SimpleLineSymbol(SimpleLineSymbolStyle.Solid, System.Drawing.Color.Red, 3));
                    _graphicsOverlay.Graphics[0].Symbol = new SimpleLineSymbol(SimpleLineSymbolStyle.Dash, System.Drawing.Color.Green, 3);
