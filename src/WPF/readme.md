...
* [Local server dynamic workspace raster](../..//Local Server/DynamicWorkspaceRaster) - Dynamically add a local raster file to a map using Local Server.
* [Local server dynamic workspace shapefile](../..//Local Server/DynamicWorkspaceShapefile) - Dynamically add a local shapefile to a map using Local Server.
* [Local server feature layer](../..//Local Server/LocalServerFeatureLayer) - Start a local feature service and display its features in a map.
* [Local server geoprocessing](../..//Local Server/LocalServerGeoprocessing) - Create contour lines from local raster data using a local geoprocessing package `.gpk` and the contour geoprocessing tool.
* [Local Server map image layer](../..//Local Server/LocalServerMapImageLayer) - Start the Local Server and Local Map Service, create an ArcGIS Map Image Layer from the Local Map Service, and add it to a map.
* [Local server services](../..//Local Server/LocalServerServices) - Demonstrates how to start and stop the Local Server and start and stop a local map, feature, and geoprocessing service running on the Local Server.
