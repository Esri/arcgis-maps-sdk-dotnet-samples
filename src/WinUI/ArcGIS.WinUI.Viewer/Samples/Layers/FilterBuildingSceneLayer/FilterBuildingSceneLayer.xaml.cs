...
namespace ArcGIS.WinUI.Samples.FilterBuildingSceneLayer
{
    ...
    public partial class FilterBuildingSceneLayer
    {
        ...
        private void PopulateCategoryTree()
        {
            ...
            // Create tree nodes for each category (e.g., Architectural, Structural, Electrical).
            foreach (BuildingGroupSublayer categorySublayer in fullModelSublayer.Sublayers)
            {
                var categoryNode = new TreeViewNode
                {
                    Content = CreateSublayerCheckBox(categorySublayer),
                    IsExpanded = false
                };

                // Add component sublayers as child nodes.
                foreach (BuildingComponentSublayer componentSublayer in categorySublayer.Sublayers)
                {
                    ...
                    {
                        Content = CreateSublayerCheckBox(componentSublayer)
                    });
                }
