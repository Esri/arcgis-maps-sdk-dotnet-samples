...
namespace ArcGIS.WinUI.Samples.FilterBuildingSceneLayer
{
    ...
    public partial class FilterBuildingSceneLayer
    {
        ...
        private void PopulateCategoryTree()
        {
            ...
            {
                // Create a tree view node for the category with a checkbox header.
                var categoryNode = new TreeViewNode
                {
                    Content = CreateCategoryHeader(categorySublayer),
                    IsExpanded = false
                };

                // Add each component sublayer as a child node.
                foreach (BuildingComponentSublayer componentSublayer in categorySublayer.Sublayers)
                {
                    var componentNode = new TreeViewNode
                    {
                        Content = CreateComponentHeader(componentSublayer)
                    };
                    categoryNode.Children.Add(componentNode);
                }

                // Add the category to the tree view.
                CategoriesTreeView.RootNodes.Add(categoryNode);
            }
        }
        ...
        {
            // Show the feature info border and hide the placeholder text.
        ...
        {
            // Hide the feature info border and show the placeholder text.
        ...
        private void CloseSelectedFeatureButton_Click(object sender, RoutedEventArgs e)
        {
            // Hide attributes UI
            FeatureInfoBorder.Visibility = Visibility.Collapsed;
            NoFeatureSelectedText.Visibility = Visibility.Visible;
            ...
            // Clear any highlighted feature in the scene
            if (_selectedSublayer != null)
            {
                _selectedSublayer.ClearSelection();
                _selectedSublayer = null;
