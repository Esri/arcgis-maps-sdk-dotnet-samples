    ...
    public class LocationDrivenGeotriggers : UIViewController
    {
        ...
        private async Task Initialize()
        {
            try
            {
                ...
                _gardenSections = ((FeatureLayer)_myMapView.Map.OperationalLayers.ToArray()[0]).FeatureTable as ServiceFeatureTable;
                _gardenPoints = ((FeatureLayer)_myMapView.Map.OperationalLayers.ToArray()[2]).FeatureTable as ServiceFeatureTable;
               ...
               if (info is FenceGeotriggerNotificationInfo fenceInfo)
               {
                   if (fenceInfo.FenceNotificationType == FenceNotificationType.Entered)
                   {
                       ...
                       if (fenceInfo.GeotriggerMonitor == _sectionMonitor)
                       {
                        ...
                        //EventLabel.Text = $"Entered {fenceInfo.Message} fence.";
                    }
                   else
                   {
                       if (fenceInfo.GeotriggerMonitor == _pointsMonitor)
                       ...
                       if (fenceInfo.GeotriggerMonitor == _sectionMonitor && _currentSection == fenceInfo.Message)
                       {
                        ...
                        //EventLabel.Text = $"Exited {fenceInfo.Message} fence.";
                    }
               }

                // Update the UI with the current section and points of interest.
                //SectionButton.Text = $"Section: {_currentSection}";

                //if (_pointsOfInterest.Any())
                //{
                //    POILabel.Text = $"Points of interest: {string.Join(", ", _pointsOfInterest)}";
                //}
                //else
                //{
        ...
        private void DisplayData(GeotriggerFeature feature)
        {
            ...
            //FeatureImage.Source = feature.ImageSource;
            //FeatureDescription.Text = feature.Description;
            //PopupPage.IsVisible = true;
        }
        ...
        //private void PopupCloseButton_Clicked(object sender, EventArgs e)
        //{
        //    PopupPage.IsVisible = false;
        //}
